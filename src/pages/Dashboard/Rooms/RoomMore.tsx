...
const RoomMore = () => {
  ...
  const { data = {}, isLoading } = GetById(QueryPATH.stacksMore, roomId, cookies.token, "/rooms")
  // Delete part
  const { mutate: RoomDelete, isPending } = Delete("/rooms", cookies.token, roomId, navigate, QueryPATH.rooms, queryClient)
  return (
    <div className="p-5">
      ...
      <div className="p-5 mb-10 flex items-start  border mt-5 border-slate-400 rounded-xl w-[38%] justify-start">
      ...
      <Modal confirmLoading={isPending} onOk={() => RoomDelete()} open={deleteModal} onCancel={() => setDeleteModal(false)} title="Do you want to delete! this room"></Modal>
    </div>
  )
}
